...
\code{Virus.Linux.Osf.8759} could not be compared to the set of benign programs because it infected the benign programs. Due to the fac that this particular sample inserted itself into the benign programs before we extracted their trace, the analysis was not relevant because the programs already contained malicious behavior. Also, it had a specific feature from the other malware samples, it opened the system call interceptor driver.
...
Our implementation succeeded in obtaining malspecs for the analyzed malware samples. The most remarcable result was for the \code{Virus.Linux.Radix} sample, although it had a shorter analysis time in comparison to ther samples. The malspec it returned consisted of two graphs.
